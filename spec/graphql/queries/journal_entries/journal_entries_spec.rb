...
RSpec.describe Types::QueryType do
  describe 'display journal entries' do
    it 'can query all journal entries' do
      ...
      je1 = create(:journal_entry, date: '2021-11-23',
                                   description: 'The frost eliminated everything',
                                   user: user1)
      ...
      expect(result['data']['journalEntries'].first['date']).to eq(je1.date)
      expect(result['data']['journalEntries'].first['description']).to eq(je1.description)
    end
  ...
  describe 'display a journal entry' do
    it 'can query one journal entry' do
      ...
      expect(result['data']['journalEntry']['date']).to eq(je1.date)
      expect(result['data']['journalEntry']['description']).to eq(je1.description)
    end
